...
function CreateTrip() {
  ...
  const onGenerateTrip = async () => {
    ...
    if (
      formData.noOfDays > 5 &&
      (!formData.Traveler || !formData.Budget || !formData.location)
    ) {
  ...
  return (
    <div className="bg-gradient-to-t from-[#272735] to-[#1b1b27] min-h-screen flex flex-col items-center">
      <div className="sm:px-10 md:px-32 lg:px-56 xl:px-72 px-5 mt-10">
        ...
        <p className="mt-3 text-gray-400 text-lg sm:text-xl text-center">
          Provide us with information, so that we can generate a perfect plan of your trip
        </p>

        <div className="mt-16 flex flex-col gap-9">
          <div>
            <h2 className="text-lg sm:text-xl my-3 font-medium text-white">
          ...
          <div>
            <h2 className="text-lg sm:text-xl my-3 font-medium text-white">
              How days are you planning your trip?
            </h2>
            <Input
              placeholder="Ex.3"
              type="Number"
        ...
        <div className="mt-10 p-5">
          <h2 className="text-lg sm:text-xl my-3 font-medium text-white">What is your budget?</h2>
          <p className="text-gray-500 text-sm sm:text-base">The budget includes only your activities and dining purpose</p>
            ...
            {SelectBudgetOptions.map((items, index) => (
              <div
                ...
                className={`text-white cursor-pointer p-4 border text-center rounded-lg transition-transform duration-300 ease-in-out hover:scale-105 hover:shadow-sm hover:shadow-purple-200 hover:border-purple-700 ${
                  formData?.Budget === items.title ? 'border border-purple-900 shadow-md shadow-purple-700' : ''
                }`}
              >
        ...
        <div className="mt-10 p-5">
          <h2 className="text-lg sm:text-xl my-3 font-medium text-white">Who are you travelling with?</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-6 flex-wrap">
            {exportTravelList.map((items, index) => (
              <div
                ...
                className={`text-white cursor-pointer p-4 border text-center rounded-lg transition-transform duration-300 ease-in-out hover:scale-105 hover:shadow-sm hover:shadow-purple-200 hover:border-purple-700 ${
                  formData?.Traveler === items.people ? 'shadow-md border-purple-900  shadow-purple-700' : ''
                }`}
              >
                <h2 className="text-4xl">{items.icon}</h2>
                <h2 className="font-bold text-lg">{items.title}</h2>
          ...
          <Button
            className="p-6 text-md sm:text-lg mt-10 bg-black hover:bg-black flex items-center justify-center"
